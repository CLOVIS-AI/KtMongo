...
val RequiredOverloadsVerifier by preparedSuite {
	...
	test("CoroutineMongoAggregationPipeline") {
		val type = CoroutineMongoAggregationPipeline::class
		...
		for (method in type.memberFunctions) {
			...
			check(method.returnType.classifier != MongoAggregationPipeline::class) { "The driver API should override all pipeline-returning methods to return a self-type instead" }
		}
	}
