...
Note the call to [`asKtMongo()`][opensavvy.ktmongo.coroutines.asKtMongo] which is the only difference from the official usage. From then on, all methods from this driver are available on the `collection` variable: see [CoroutineMongoCollection][opensavvy.ktmongo.coroutines.CoroutineMongoCollection].
