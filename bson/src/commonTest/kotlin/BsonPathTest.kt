...
val BsonPathTest by preparedSuite {
	...
	suite("Parsing") {
		...
		test("Parse a combined field names") {
			check(BsonPath("$.foo['bar'][\"baz\"]").toString() == "$.foo.bar.baz")
			check(BsonPath("$.foo['bar'][\"baz\"]") == BsonPath["foo"]["bar"]["baz"])
		}

		test("Parse a simple array index") {
			check(BsonPath("$[0]").toString() == "$[0]")
			check(BsonPath("$[0]") == BsonPath[0])
		}

		test("Parse a negative array index") {
			check(BsonPath("$[-1]").toString() == "$[-1]")
			check(BsonPath("$[-1]") == BsonPath[-1])
		}

		test("Parse all fields") {
			check(BsonPath("$.foo[*]").toString() == "$.foo.*")
			check(BsonPath("$.foo[*]") == BsonPath["foo"].all)
		}
		...
		test("Slice without bounds") {
			check(BsonPath("$[:]").toString() == "$[:]")
			check(BsonPath("$[:]") == BsonPath.sliced())
			check(BsonPath("$[:]") == BsonPath.sliced(0..Int.MAX_VALUE))
		}

		test("Reversing slice") {
			check(BsonPath("$[::-1]").toString() == "$[::-1]")
			check(BsonPath("$[::-1]") == BsonPath.sliced(step = -1))
			check(BsonPath("$[::-1]") == BsonPath.reversed())
		}

		test("IntProgression cannot be used to reverse a slice") {
			check(BsonPath("$[2147483647::-1]") == BsonPath.sliced(Int.MAX_VALUE downTo 0 step 1))
		}

		test("Slice with minimum") {
			check(BsonPath("$[5:]").toString() == "$[5:]")
			check(BsonPath("$[5:]") == BsonPath.sliced(start = 5))
			check(BsonPath("$[5:]") == BsonPath.sliced(5..Int.MAX_VALUE))
		}

		test("Slice with maximum") {
			check(BsonPath("$[:5]").toString() == "$[:5]")
			check(BsonPath("$[:5]") == BsonPath.sliced(end = 5))
			check(BsonPath("$[:5]") == BsonPath.sliced(0..<5))
		}

		test("Slice with minimum and maximum") {
			check(BsonPath("$[2:4]").toString() == "$[2:4]")
			check(BsonPath("$[2:4]") == BsonPath.sliced(2, 4))
			check(BsonPath("$[2:4]") == BsonPath.sliced(2..<4))
		}

		test("Slice with step") {
			check(BsonPath("$[1:7:2]").toString() == "$[1:7:2]")
			check(BsonPath("$[1:7:2]") == BsonPath.sliced(1, 7, 2))
			check(BsonPath("$[1:6:2]") == BsonPath.sliced(1..<7 step 2)) // 1:7:2 is 1,3,5 (because 7 is an exclusive bound), the Kotlin IntProgression class simplifies the range
		}

		test("Slice with negative step") {
			check(BsonPath("$[7:1:-1]").toString() == "$[7:1:-1]")
			check(BsonPath("$[7:1:-1]") == BsonPath.sliced(7, 1, -1))
			check(BsonPath("$[7:1:-1]") == BsonPath.sliced(7 downTo 2 step 1))
		}

		test("Slice with negative step of 2") {
			check(BsonPath("$[7:1:-2]").toString() == "$[7:1:-2]")
			check(BsonPath("$[7:1:-2]") == BsonPath.sliced(7, 1, -2))
			check(BsonPath("$[7:2:-2]") == BsonPath.sliced(7 downTo 2 step 2)) // 7:1:-2 is 7,5,3 (because 1 is an exclusive bound), the Kotlin IntProgression class simplifies the range
		}

		test("Default slice with negative step") {
			check(BsonPath("$[::-2]").toString() == "$[::-2]")
			check(BsonPath("$[::-2]") == BsonPath.sliced(step = -2))
		}
